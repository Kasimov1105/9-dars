...
const RatingStars = ({ rating }: { rating: number }) => {
  const stars = Array(5).fill(false).map((_, index) => index < Math.round(rating));
  return (
    <div className="flex items-center">
      {stars.map((filled, index) => (
        <svg key={index} className={`w-4 h-4 ${filled ? 'text-yellow-300' : 'text-gray-200'} dark:text-gray-600`} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
